...
PDBLoader.prototype = Object.assign( Object.create( Loader.prototype ), {
	...
	parse: function ( text ) {
		...
		for ( var i = 0 , maxim = index; i<maxim; i++ ){
			var counter = 0
			x = atoms[i][0];
			y = atoms[i][1];
			z = atoms[i][2];
			if ( Math.abs(x) < 0.05) {
			    var add_index = maxim + 1;
				maxim = add_index;
				
				var atomData = [ x+3.915, y, z, atoms[i][3], atoms[i][4] ];

				console.log('Hello x from index ', i,  'is around 0')
				atoms.push( atomData );
			};
			...
			if ( Math.abs(y) < 0.05) {
				...
				var atomData = [ x, y+3.915, z, atoms[i][3], atoms[i][4] ];

				console.log('Hello y from index ', i,  'is around 0')
				atoms.push( atomData );
			}
			...
			if ( Math.abs(z) < 0.05) {
				...
				var atomData = [ x, y, z+3.915, atoms[i][3], atoms[i][4] ];

				console.log('Hello z from index ', i,  'is around 0')
				atoms.push( atomData );
			}
			
			console.log(_atomMap[i]);
		}

		// build and return geometry
		console.log('logging atoms')
		console.log(atoms)
